...
\begin{itemize}
  \item \textbf{Accuracy} -- $\frac{TP+TN}{TP+TN+FP+FN}$ -- celková přesnost
  \item \textbf{Precision} -- $\frac{TP}{TP+FP}$ -- z predikovaných pozitiv, kolik je správně
  \item \textbf{Recall (Sensitivity)} -- $\frac{TP}{TP+FN}$ -- z reálných pozitiv, kolik bylo nalezeno
  \item \textbf{Specificity} -- $\frac{TN}{TN+FP}$ -- z reálných negativ, kolik bylo správně klasifikováno
  \item \textbf{F1 score} -- $2 \cdot \frac{Precision \cdot Recall}{Precision + Recall}$ -- harmonický průměr
\end{itemize}

\paragraph{ROC křivka (Receiver Operating Characteristic)}
Graf závislosti TPR (Recall) na FPR ($\frac{FP}{FP+TN}$) při různých prahových hodnotách.
\textbf{AUC} (Area Under Curve) -- plocha pod ROC křivkou; AUC = 1 = perfektní model; AUC = 0.5 = náhodný model.
...
\begin{itemize}
  ...
  \item \textbf{IDF (Inverse Document Frequency)} $= \log\dfrac{N}{df(t)}$, kde $N$ = počet dokumentů,
    $df(t)$ = v kolika dokumentech se term vyskytuje; běžná slova jako ``a'', ``the'' jsou ve všech
    dokumentech $\Rightarrow$ IDF $\approx 0$ $\Rightarrow$ váha blízká nule (filtrují se automaticky)
\end{itemize}
