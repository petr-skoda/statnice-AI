...
\textbf{Governance} stanovuje \textit{co} a \textit{proč} -- určuje cíle, priority a pravidla hry;
zodpovídá za ni rada či vedení organizace.
\textbf{Management} řeší \textit{jak} -- plánuje, realizuje a provozuje konkrétní aktivity, aby těchto cílů dosáhl;
zodpovídají za něj manažeři a vedoucí týmů.
Governance je tedy nadřazená vrstva: vymezuje prostor, ve kterém management pracuje.
...
rozšířená a v literatuře hojně citovaná.

\textbf{Pro koho je COBIT určen?}
Primárně pro \textbf{management a vedení organizace} -- CIO, IT ředitele, interní auditory a compliance manažery.
Zajímá je otázka \textit{„Děláme správné věci a dosahujeme cílů?"} -- tedy governance pohled na IT.
Typicky ho používají větší organizace, banky, pojišťovny a firmy podléhající regulaci (SOX, Basel).

\textbf{Vztah COBIT a ITIL:}
COBIT a ITIL se doplňují -- nejsou konkurenti.
\begin{itemize}
  \item \textbf{COBIT} říká \textit{co} musí governance a management IT pokrývat a jak měřit, zda je to splněno; je to zastřešující rámec.
  \item \textbf{ITIL} říká \textit{jak} konkrétně provozovat a dodávat IT služby; je to provozní návod pro IT tým.
  \item Prakticky: COBIT definuje cíl (např. ``Zajistit dodávku kvalitních služeb''), ITIL poskytuje procesy, jak ho naplnit (Incident Management, Change Management\ldots).
  \item COBIT sám na ITIL odkazuje jako na jeden z doporučených standardů pro oblast správy IT služeb.
\end{itemize}
...
\textbf{Facilitátor (enabler)} je cokoliv, co pomáhá organizaci dosáhnout svých cílů v oblasti governance a managementu IT -- tedy zdroje, nástroje a mechanismy, které governance \textit{umožňují} fungovat.
COBIT definuje 7 takových facilitátorů:
\begin{enumerate}
...
Toto je nejdůležitější rozlišení v ITIL:
...
\begin{itemize}
  ...
  \item \textbf{4 dimenze správy služeb} -- každou službu je nutné zvážit ze čtyř úhlů pohledu,
    aby nic nebylo opomenuto:
    \begin{enumerate}
      \item \textbf{Organizace a lidé} -- kdo službu zajišťuje; role, zodpovědnosti, kultura a kompetence týmu
      \item \textbf{Informace a technologie} -- jaká data, znalosti a nástroje jsou ke službě potřeba
      \item \textbf{Partneři a dodavatelé} -- kdo dodává zvenčí; smlouvy, závislosti na třetích stranách
      \item \textbf{Procesy a hodnotové toky} -- jak práce protéká organizací od požadavku k výsledku; aktivity a jejich návaznosti
    \end{enumerate}
...
%=============================================================================
\subsection{Řízení portfolia IT projektů (IT PPM)}
%=============================================================================

\textbf{IT Project Portfolio Management (IT PPM)} je disciplína správy a optimalizace sady IT projektů
a programů jako celku, nikoli individuálně.
Cílem je maximalizovat celkovou hodnotu IT investic při omezeném rozpočtu a kapacitě zdrojů
a zajistit soulad projektů se strategií organizace.
...
\midrule
\textbf{Projekt} & Dočasné úsilí za účelem vytvoření unikátního výsledku (PMBOK) \\
\textbf{Program} & Skupina vzájemně souvisejících projektů řízených koordinovaně;
  benefity nelze dosáhnout jednotlivými projekty samostatně \\
\textbf{Portfolio} & Soubor projektů, programů a operativního úsilí sdružených ke strategickým cílům;
  projekty v portfoliu nemusejí spolu přímo souviset \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Proč řídit portfolio, ne jen jednotlivé projekty?}
Bez PPM firmy trpí typickými problémy: příliš mnoho projektů najednou přetěžuje zdroje a nic se nedokončí;
projekty bez strategické hodnoty ``přežívají'' kvůli politickým tlakům; duplicitní projekty řeší stejný problém
různými týmy; portfolio není schopno reagovat na změnu strategie.

\textit{Příklad:} IT oddělení banky má 40 otevřených projektů. Bez PPM každý projekt bojuje o vývojáře zvlášť.
S PPM management vidí celé portfolio a rozhodne: ``Pozastavíme 15 nízkoprioritních projektů
a uvolníme kapacity pro strategický projekt open bankingu.''

\subsubsection{Kategorizace portfolia -- Run/Grow/Transform}

Gartnerovo rozdělení IT výdajů a projektů do tří kategorií:

\begin{center}
\begin{tabular}{p{2.8cm}p{4.2cm}p{5.2cm}}
\toprule
\textbf{Kategorie} & \textbf{Popis} & \textbf{Příklady} \\
\midrule
\textbf{Run} \newline (Provoz) &
  Udržet stávající systémy v chodu; snižovat náklady; bez těchto aktivit firma nefunguje &
  Provoz ERP, zálohy, helpdesk, licence, bezpečnostní patche \\
\midrule
\textbf{Grow} \newline (Růst) &
  Rozšíření stávajících schopností; incrementální inovace; podpora business růstu &
  Nový modul CRM, integrace e-commerce, rozšíření DWH \\
\midrule
\textbf{Transform} \newline (Transformace) &
  Disruptivní změny; zcela nové schopnosti; vysoké riziko i potenciální hodnota &
  Migrace do cloudu, zavedení AI, digitální transformace \\
\bottomrule
\end{tabular}
\end{center}

\textit{Typické rozložení:} Run 60--70\,\%, Grow 20--25\,\%, Transform 10--15\,\%.
Strategicky vyspělé firmy tlačí podíl Transform nahoru; Run nelze zanedbat.

\subsubsection{Procesy řízení IT portfolia}

\paragraph{1. Identifikace a registrace}
Všechny navrhované IT projekty jsou evidovány v \textbf{registru portfolia}.
Každý projekt má Business Case: co řeší, odhadované náklady (TCO), přínosy a rizika.

\paragraph{2. Hodnocení a selekce}

\begin{center}
\begin{tabular}{p{3.5cm}p{8.5cm}}
\toprule
\textbf{Kritérium} & \textbf{Popis} \\
\midrule
\textbf{Strategická shoda} & Do jaké míry projekt podporuje podnikové nebo IT cíle? (skóre 1--5) \\
\textbf{Finanční hodnota} & ROI, NPV (čistá současná hodnota), doba návratnosti \\
\textbf{Riziko} & Technické, organizační a dodavatelské riziko \\
\textbf{Regulatorní povinnost} & Je projekt povinný (zákon, GDPR, DORA)? Povinné projekty mají automaticky vysokou prioritu \\
\textbf{Závislosti} & Závisí projekt na jiných projektech nebo systémech? \\
\textbf{Kapacitní dopady} & Kolik FTE projekt vyžaduje a na jak dlouho? \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{3. Prioritizace a schválení}
Projekty jsou seřazeny pomocí \textbf{scoring modelu} (vážený průměr kritérií) a předloženy
\textbf{IT řídícímu výboru} (IT Steering Committee) nebo \textbf{PMO} ke schválení.
Výsledkem jsou tři skupiny: schválené a financované projekty, projekty v zásobníku (backlog)
a odmítnuté projekty (nízká priorita nebo nesoulad se strategií).

\paragraph{4. Monitoring a rebalancování}
Portfolio se přehodnocuje pravidelně (čtvrtletně):
\begin{itemize}
  \item Změnila se strategie? $\to$ přehodnotit priority
  \item Projekt zaostává za plánem? $\to$ rozhodnout: pokračovat, pozastavit, ukončit
  \item Uvolnily se zdroje? $\to$ aktivovat projekty z backlogu
\end{itemize}

\subsubsection{Val IT -- rámec pro řízení hodnoty IT investic}

\textbf{Val IT} (ISACA, 2006/2008) je rámec doplňující COBIT, zaměřený na otázku:
\textit{``Investujeme do správných věcí a dostáváme za to odpovídající hodnotu?''}

\begin{center}
\begin{tabular}{p{4cm}p{8cm}}
\toprule
\textbf{Doména Val IT} & \textbf{Obsah} \\
\midrule
\textbf{Value Governance (VG)} &
  Governance IT investic: principy, role, odpovědnosti; IT Investment Board \\
\textbf{Portfolio Management (PM)} &
  Správa celého portfolia: selekce, prioritizace, rebalancování; transparentnost investic \\
\textbf{Investment Management (IM)} &
  Životní cyklus jednotlivé investice: Business Case $\to$ realizace $\to$ realizace přínosů \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Klíčový koncept -- Benefits Realization (realizace přínosů):}
Nestačí projekt dokončit -- přínosy musejí být aktivně sledovány \textit{po} nasazení.
Zodpovědnost za přínos nese \textbf{business owner} (ne IT), protože hodnotu generuje
změna business procesů, nikoli technologie samotná.

\textit{Příklad:} Projekt nového CRM stojí 5 mil.\ Kč. Business Case předpokládá 15\,\% nárůst konverzního poměru.
Val IT vyžaduje: (a) změřit baseline před projektem, (b) monitoring KPI po 6 a 12 měsících,
(c) formální review -- bylo 15\,\% dosaženo? Pokud ne, proč a co s tím?

\subsubsection{Vztah PPM ke COBIT}

V COBIT 5/2019 je řízení portfolia součástí domény \textbf{APO (Align, Plan, Organise)}:
\begin{itemize}
  \item \textbf{APO05 -- Manage Portfolio:} definovat investiční mix, hodnotit a vybírat programy,
    monitorovat a rebalancovat portfolio
  \item \textbf{APO06 -- Manage Budget and Costs:} finanční řízení IT portfolia, TCO sledování
  \item \textbf{BAI01 -- Manage Programmes and Projects:} metodické řízení programů a projektů v portfoliu
\end{itemize}

COBIT tak propojuje governance IT (EDM -- co schvalujeme) s plánováním portfolia (APO05)
a realizací projektů (BAI01).
...
\end{itemize}
\textbf{Maryška Miloš, doc. Ing., Ph.D. -- IT PPM}
\begin{itemize}
  \item Co je IT Project Portfolio Management a jaký je rozdíl od řízení jednotlivých projektů?
  \item Jaká kritéria se používají pro selekci a prioritizaci IT projektů v portfoliu?
  \item Co je Val IT a co znamená ``Benefits Realization''?
  \item Jak kategorizujeme IT výdaje/projekty dle Gartnera (Run/Grow/Transform)?
\end{itemize}
\textbf{Svatá Vlasta, doc. Ing., Ph.D.}
\begin{itemize}
  \item Co je procesní řízení a jaký má vztah k IT governance?
  \item Jaké jsou modely dodávek ERP systémů?
  \item Co je workflow a k čemu slouží?
  \item Co je IT Governance a jaké jsou její klíčové aspekty?
  \item Co je teorie rizik?
  \item Co je mapa rizik a jak se sestavuje?
  \item Jaký je rozdíl mezi kvantitativními a kvalitativními metodami hodnocení rizik?
\end{itemize}
\textbf{Rožmberská Lucie, Ing.}
\begin{itemize}
  \item Co je ITIL a jaké jsou jeho základní principy?
  \item Co je RPA (Robotic Process Automation) a jak se využívá?
  \item Co je SLA a co definuje?
\end{itemize}
\textbf{Sigmund Tomáš, doc. Ing., Ph.D.}
\begin{itemize}
  \item Jaké jsou bezpečnostní aspekty z pohledu IT governance?
  \item Jaké jsou aspekty GDPR v oblasti IT?
  \item Jak se řídí IT na různých úrovních organizace?
  \item Jak se zavádí systém na podporu rozhodování?
\end{itemize}
\textbf{Komise: Sládek, Chudán, Svatá}
\begin{itemize}
  \item Jaké jsou základní funkce ERP?
  \item Jak AI ovlivňuje IT governance?
  \item Co je IT governance a čím se liší od IT managementu?
  \item Jaké jsou klíčové procesy ITIL?
\end{itemize}
\textbf{Komise: Kliegr, Potančok, Sedláček}
\begin{itemize}
  \item Jaké jsou klíčové oblasti IT managementu?
  \item Jaký je rozdíl mezi ITIL 4 a ITIL 3?
\end{itemize}
\textbf{Komise: Zamazal, Karkošková, Hološka}
\begin{itemize}
  \item Jaké znáte normální formy databází?
  \item Co je ITIL a jaké jsou jeho fáze?
  \item Co je Business Intelligence a k čemu slouží?
\end{itemize}
\end{profesorbox}
