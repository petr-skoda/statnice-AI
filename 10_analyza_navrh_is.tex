...
\begin{enumerate}
  \item \textbf{Strategy Execution} -- business strategie $\to$ org. infrastruktura $\to$ IT infrastruktura;
    IT podporuje realizaci definované business strategie (nejčastější pohled)
  \item \textbf{Technology Transformation} -- business strategie $\to$ IT strategie $\to$ IT infrastruktura;
    IT odvozuje svou strategii z businessu a buduje odpovídající infrastrukturu
  \item \textbf{Competitive Potential} -- IT strategie $\to$ business strategie $\to$ org. infrastruktura;
    IT umožní nové business strategie (IT jako hybná síla inovace)
  \item \textbf{Service Level} -- IT strategie $\to$ IT infrastruktura $\to$ org. infrastruktura;
    IT jako excelentní servisní organizace
\end{enumerate}
...
Rámec ve formě matice $6 \times 6$ -- neříká \textit{jak} architektonické práce dělat (na rozdíl od TOGAF),
ale \textit{co} musí být popsáno.
...
Pozn.: VY = výsledky (ne fáze); každá fáze produkuje artefakty.
...
\begin{itemize}
  \item \textbf{Sequence Flow} -- plná šipka; tok řízení mezi objekty \textit{(př.: šipka z „Ověřit objednávku" na „Vystavit fakturu")}
  \item \textbf{Message Flow} -- přerušovaná šipka; komunikace mezi pooly \textit{(př.: zákazník posílá objednávku do poolu e-shopu)}
...
\vspace{0.4cm}
\noindent\textbf{BPMN -- orientace zleva doprava, explicitní Start/End Event:}

\begin{center}
\begin{tikzpicture}[
  node distance=1.0cm and 1.5cm,
  startevent/.style={draw, circle, minimum size=0.75cm, thin},
  endevent/.style={draw, circle, minimum size=0.75cm, line width=2.5pt},
  task/.style={draw, rounded corners=4pt, minimum width=2.8cm, minimum height=0.85cm,
               align=center, font=\scriptsize, fill=blue!10},
  gw/.style={draw, shape=diamond, minimum width=1.1cm, minimum height=1.1cm,
             font=\scriptsize\bfseries, fill=orange!15, inner sep=0pt},
  arr/.style={->, >=stealth}
]
\node[startevent, label=below:{\tiny Objednávka přijata}] (start) {};
\node[task, right=of start] (t1) {Zkontrolovat\\dostupnost zboží};
\node[gw, right=of t1] (xor) {X};
\node[task, above right=0.9cm and 1.2cm of xor] (t2) {Připravit zásilku};
\node[task, below right=0.9cm and 1.2cm of xor] (t3) {Informovat zákazníka};
\node[endevent, right=of t2, label=below:{\tiny Zásilka odeslána}] (end1) {};
\node[endevent, right=of t3, label=below:{\tiny Zákazník informován}] (end2) {};
\draw[arr] (start) -- (t1);
\draw[arr] (t1) -- (xor);
\draw[arr] (xor) -- (t2) node[midway, above, font=\tiny] {skladem};
\draw[arr] (xor) -- (t3) node[midway, below, font=\tiny] {není};
\draw[arr] (t2) -- (end1);
\draw[arr] (t3) -- (end2);
\end{tikzpicture}
\end{center}
...
\textbf{MDA} (OMG standard) -- přístup k vývoji SW, kde je model primárním artefaktem, nikoliv kód.

\begin{itemize}
  \item \textbf{CIM (Computation Independent Model)} -- model požadavků a podnikových procesů; nezávislý na technologii
